...
def enhance_zoom_figure(fig_dict):
    ...
    if isinstance(layout, dict):
        ...
        if "title" in layout and isinstance(layout["title"], dict):
            ...
            layout["title"]["font"]["size"] = max(int(layout["title"]["font"].get("size", 18)), 24)
            layout["title"]["x"] = 0.5

        layout.setdefault("legend", {})
        if isinstance(layout["legend"], dict):
            layout["legend"].setdefault("font", {})
            layout["legend"]["font"]["size"] = max(int(layout["legend"]["font"].get("size", 12)), 14)
...
app.index_string = app.index_string.replace(
    "</head>",
    f"<style>{DROPDOWN_FIX_CSS}</style></head>"
)
...
app.layout = dbc.Container(fluid=True, children=[
    ...
    # NOTE: XOÁ 3 Store dt-year/lh-year/hd-year vì trùng ID với Dropdown (gây lỗi ngầm)
...
def callbacks(prefix, df, value_col):
    ...
    if prefix == "lh":
        p1_filter_input = Input("lh-type-p1", "value")
        p2_filter_input = Input("lh-type-p2", "value")
    elif prefix == "hd":
        p1_filter_input = Input("hd-type-p1", "value")
        p2_filter_input = Input("hd-type-p2", "value")

    inputs_p1 = [Input(f"{prefix}-year", "value"), Input(f"{prefix}-month", "value"), Input("theme", "data")]
    ...
    def p1(*args):
        ...
        months = args[1]
        theme = args[2]
        type_filter = args[3] if len(args) > 3 else None
    ...
    inputs_p2 = [Input(f"{prefix}-dim","value"), Input(f"{prefix}-year-p2","value"), Input(f"{prefix}-month-p2","value"), Input("theme","data")]
    ...
    def p2(*args):
        ...
        months = args[2]
        theme = args[3]
        type_filter = args[4] if len(args) > 4 else None
...
def answer_question(question: str) -> str:
    ...
    intent = detect_metric_intent(question, value_col_default)
    metric_col = intent["metric_col"]
    ...
    total = dff[metric_col].sum()
    return f"Tổng **{metric_name}**{yr_text}{mo_text}{tf_text} là **{fmt_vn(total)}**."
