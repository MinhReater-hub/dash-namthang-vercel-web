...
def render(menu,page):
    """
    Render *all* pages in the DOM (hide inactive ones).
    This prevents Dash from throwing "nonexistent Output" errors when global inputs
    like theme trigger callbacks for components not currently visible.
    """
    def wrap(children, show):
        return html.Div(children, style={"display": "block" if show else "none"})

    # Safety: normalize page to int (can come as string from dcc.Store)
    try:
        p = int(page) if page is not None else 1
    except Exception:
        p = 1

    # Build all pages once per render call, then hide/show via wrapper style.
    return html.Div([
        wrap(page_1("dt", df_dt, "Doanh thu", "Số chuyến"), menu=="dt" and p==1),
        wrap(page_2("dt", df_dt, "Doanh thu", "Số chuyến"), menu=="dt" and p==2),
        wrap(page_1("lh", df_lh, "Doanh thu", "Số chuyến", has_type=True), menu=="lh" and p==1),
        wrap(page_2("lh", df_lh, "Doanh thu", "Số chuyến", has_type=True), menu=="lh" and p==2),
        wrap(page_1("hd", df_hd, "Doanh thu", "Số chuyến"), menu=="hd" and p==1),
        wrap(page_2("hd", df_hd, "Doanh thu", "Số chuyến"), menu=="hd" and p==2),
    ])
...
def answer_question(question: str, context: dict | None = None) -> str:
    """
    AI trả lời dựa trên dữ liệu dashboard (rule-based, có ngữ cảnh).
    - Tự suy luận dataset theo câu hỏi (hoặc theo menu/page hiện tại nếu câu hỏi không nói rõ)
    - Hỗ trợ nhiều kiểu hỏi: tổng, top, so sánh, xu hướng theo tháng, tỷ trọng, trung bình
    - Ưu tiên dùng filter hiện tại (năm/tháng/khu vực/loại hình/hạng đơn) nếu người dùng không chỉ định
    """
    context = context or {}
    q_raw = (question or "").strip()
    if not q_raw:
        return "Bạn hãy nhập câu hỏi (ví dụ: *Doanh thu T01/2025 khu vực HCM?*)"
    ...
    def _choose_dataset_with_context(q_norm: str):
        # nếu câu hỏi có nhắc rõ, dùng heuristic cũ; nếu không thì ưu tiên menu hiện tại
        explicit = any(k in q_norm for k in ["hop dong", "hđ", "hd ", "hang don", "don hang"]) or                    any(k in q_norm for k in ["loai hinh", "lh ", "khach hang", "kh"])
        if explicit:
            ds_key, df = choose_dataset(q_norm)
            return ds_key, df
    ...
    def _pick_type_value(df: pd.DataFrame, col: str, q_norm: str):
        if col not in df.columns:
            return None
        vals = [v for v in df[col].dropna().astype(str).unique().tolist() if str(v).strip()]
        if not vals:
            return None
        # match theo chuỗi chuẩn hoá (ưu tiên full token)
        ...
        for v in vals:
            ...
            if not vn2:
                continue
            if vn2 in q2 and len(vn2) > best_len:
                best = v
                best_len = len(vn2)
        return best

    # ----- choose dataset + defaults from context -----
    ...
    # metric intent
    metric = detect_metric_intent(q_raw)  # returns REV_COL or TRIP_COL
    ...
    ctx_regions = ctx_filters.get("dim") or ctx_filters.get("dims")  # page2 vùng
    if isinstance(ctx_regions, str):
        ctx_regions = [ctx_regions]

    # extract time / region from question
    years = _extract_years(qn)
    month_pairs = _extract_month_pairs(qn)

    # range: "tu 01/2025 den 06/2025"
    range_labels = []
    ...
    if range_labels:
        ...
        # fallback: nhận dạng "Txx/yyyy" theo helper cũ
    ...
    # type filter (LH/HD) from question or context
    type_col = None
    if ds_key == "lh":
        type_col = LH_COL
    elif ds_key == "hd":
    ...
    if not type_value and type_col:
        # context: lh-type-p1 / hd-type-p1 dùng key "type"
        tv = ctx_filters.get("type")
        if tv:
            type_value = tv
    ...
    if type_col and type_value and type_col in dff.columns:
        dff = dff[dff[type_col] == type_value]
    ...
    if dff.empty:
        ...
        if type_value: scope.append(f"{type_col} = {type_value}")
    ...
    # ----- answers -----
    def _fmt_value(v):
        return fmt_vn(v, is_money=(metric == REV_COL))
    ...
    if type_value: f_desc.append(f"{type_col}: {type_value}")
    if f_desc:
        parts.append("**Bộ lọc:** " + " | ".join(f_desc))

    # Average / ratio
    if any(k in qn for k in ["trung binh", "tb", "avg", "average"]):
        # avg per trip if revenue available
    ...
    # Compare (regions or years)
    if any(k in qn for k in ["so sanh", "so với", "vs", "versus", "khac nhau", "chenh", "chênh"]):
        # compare years if 2+ years
        if YEAR_COL in dff.columns and len(_extract_years(qn)) >= 2:
            yrs = _extract_years(qn)[:3]
            tmp = df.copy()
        ...
        if REGION_COL in dff.columns and len(regions) >= 2:
            ...
            if len(lines) >= 2:
                ...
                diff = v0 - v1
                pct = (diff / v1 * 100.0) if v1 else None
                if pct is not None:
                    parts.append(f"**Chênh lệch {r0} so với {r1}:** {_fmt_value(diff)} ({pct:+.1f}%)")
                else:
    ...
    if intent == "trend":
        if DATE_COL in dff.columns:
            ...
            lines = []
            # show up to 12 points, head+tail if long
            if len(s) <= 12:
                show = s
            else:
                show = pd.concat([s.head(6), s.tail(6)])
            for d, v in show.items():
                lab = _month_label(int(pd.Timestamp(d).month), int(pd.Timestamp(d).year))
    ...
    # add quick breakdown if multiple regions and not explicitly asked
    ...
    # hint
    parts.append('\n*Gợi ý:*Bạn có thể hỏi kiểu: **"So sánh doanh thu 2024 vs 2025"**, **"Top 10 số cuốc T01/2026"**, **"Xu hướng doanh thu từ 01/2025 đến 06/2025"**, **"Tỷ trọng theo khu vực"**.')
    return "\n\n".join(parts)
...
def ai_chat(n_send, n_clear, question, history,
    ...
    def split_questions(raw: str):
        ...
        # tránh spam quá nhiều câu trong 1 lần
        return parts[:8]
    ...
    # render last few turns as markdown
    md_lines = []
